...
var footer = ['</urlset>'];
var allowedProperties = ['getLoc', 'lastmod', 'priority', 'changefreq', 'hreflang','comment'];

function prepareHeader(comment,entriesHref) {
    var c,
        h = [];
    if (comment != "") {
        c = '<!-- ' + comment + ' -->';
...
function prepareSitemap(entries, config) {
    ...
    return (prepareHeader(config.comment,entriesHref))
        // .concat((entriesHref ? headerHref : header))
